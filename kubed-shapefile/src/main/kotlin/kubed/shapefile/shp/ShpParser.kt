...
fun read(shp: File): FeatureCollection {
    val features = ArrayList<Feature>()
    MixedEndianInputStream(shp.inputStream().buffered()).use { stream ->
        try {
            stream.skipBytes(100)

            while(true) {
                val shapeType = stream.readIntLe()
                when(shapeType) {
                    1 -> features += Feature(parsePoint(stream))
                    3 -> features += Feature(parsePolyline(stream))
                    5 -> features += Feature(parsePolygon(stream))
                    8 -> features += Feature(parseMultiPoint(stream))
                }
            }
        }
...
private fun parsePolygon(stream: MixedEndianInputStream): Geometry<*> {
    ...
    parts.forEachIndexed { j, i ->
        ...
        if(ringClockwise(ring)) polygons.add(listOf(ring))
        else holes.add(ring)
    }
...
fun main(args: Array<String>) {
    val features = read(File("/Users/hudsonb/TM_WORLD_BORDERS_SIMPL-0.3/TM_WORLD_BORDERS_SIMPL-0.3.shp"))
