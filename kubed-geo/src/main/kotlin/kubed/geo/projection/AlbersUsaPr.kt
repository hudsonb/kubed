...
fun albersUsaPr() = albersUsaPr {}
fun albersUsaPr(init: AlbersUsaPr.() -> Unit) = AlbersUsaPr().apply {
    scale = 1070.0
    init()
}

open class AlbersUsaPr : StreamCacheProjection() {
    ...
    private val puertoRico = conicEqualArea {
        rotateX = 157.0
        center = Position(-3.0, 19.9)
        parallels = doubleArrayOf(8.0, 18.0)
        scale = lower48.scale
    }
    lateinit var puertoRicoPoint: GeometryStream
    ...
    private fun update() {
        ...
        with(puertoRico) {
            translateX = x + .350 * k
            translateY = y + .224 * k
            clipExtent = Rectangle2D(x + .320 * k, y + .204 * k, 0.06 * k, 0.03 * k)
            puertoRicoPoint = stream(pointStream)
        }
    ...
    override fun invoke(point: DoubleArray): DoubleArray {
        ...
        puertoRicoPoint.point(x, y, 0.0)
        val prp = this.point
        if(prp != null) return prp

        throw Exception("Bug in AlbersUsaPr.invoke")
    }
    ...
    override fun invert(coordinates: DoubleArray): DoubleArray {
        ...
        val p = when {
            ...
            y in 0.204..0.234 && x in 0.320..0.380 -> puertoRico
            else -> lower48
        }
