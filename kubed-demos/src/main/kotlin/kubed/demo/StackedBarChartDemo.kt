...
class StackedBarChartDemo: Application() {
    override fun start(primaryStage: Stage?) {
        ...
        val bar = rect<Point<Map<String, String>, String>> {
            ...
            translateX { d, _ -> xScale(d.data["State"] as String) }
            translateY { d, _ -> yScale(d.y1) }
            height { d, _ -> yScale(d.y0) - yScale(d.y1) }
            width(xScale.bandwidth)
        }

        root.selectAll<Series<*, *>>("Group")
                .data(stack({ keys }, { d: Map<String, String>, k: String -> d[k]?.toDouble() as Double}, data))
                .enter().append { _, _, _ -> Group() }
                .selectAll<Point<Map<String, String>, String>>("Rectangle")
                .data<Series<Map<String, String>, String>>({d, _, _ -> d as List<Point<Map<String, String>, String>> })
                .enter()
                .append { d, _, _ -> bar(d) }
